...
class ParadoxScriptSchemaCompletionContributor : CompletionContributor() {
	...
	private class Worker(
		...
		private val isInsideStringLiteral = position.parent is ParadoxScriptStringValue
		...
		private fun mayBePropertyKey(checkable: PsiElement): Boolean {
			return checkable is ParadoxScriptString || checkable is ParadoxScriptNumber
			       && checkable.parent !is ParadoxScriptPropertyValue
		}
		...
		private fun addInjectedLanguageVariants() {
			...
			if(checkable !is ParadoxScriptString) return
		...
		private fun addRequiredPropVariants() {
			val checkable = walker.findElementToCheck(position)
			if(checkable !is ParadoxScriptStringValue) return
			...
			val items = when(val parent = checkable.parent) {
				is ParadoxScriptBlock -> parent.valueList.filterIsInstance<JsonStringLiteral>().map { it.value }.toSet()
				else -> setOf()
			}
		...
		companion object {
			...
			private fun handleIncompleteString(editor: Editor, element: PsiElement): Boolean {
				if((element as LeafPsiElement).elementType === TokenType.WHITE_SPACE) {
					...
					if(prevSibling is ParadoxScriptProperty) {
