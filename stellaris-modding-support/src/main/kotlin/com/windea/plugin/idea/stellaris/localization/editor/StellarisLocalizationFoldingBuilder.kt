...
class StellarisLocalizationFoldingBuilder : FoldingBuilder, DumbAware {
	override fun getPlaceholderText(node: ASTNode): String? {
		return when(node.elementType){
			StellarisLocalizationTypes.ROOT_COMMENT -> "#..."
			StellarisLocalizationTypes.COMMENT -> "#..."
	...
	private fun collectDescriptorsRecursively(node: ASTNode, document: Document, descriptors: MutableList<FoldingDescriptor>) {
		var startOffset: Int
		var endOffset: Int
		if(node.elementType == StellarisLocalizationTypes.COMMENT || node.elementType == StellarisLocalizationTypes.ROOT_COMMENT) {
