...
public class JsonSchemaCompletionContributor0 extends CompletionContributor {
	...
	public void fillCompletionVariants(@NotNull CompletionParameters parameters, @NotNull CompletionResultSet result) {
		...
		if(positionParent != null) {
			...
			if(parent instanceof JsonProperty) {
				...
				if("$schema".equals(propName) && parent.getParent() instanceof JsonObject && parent.getParent()
					.getParent() instanceof JsonFile
	...
	public static void doCompletion(@NotNull final CompletionParameters parameters,
		...
		@NotNull final JsonSchemaObject rootSchema,
		boolean stop) {
		final PsiElement completionPosition =
			parameters.getOriginalPosition() != null ? parameters.getOriginalPosition() :
	...
	private static void updateStat(@Nullable JsonSchemaFileProvider provider, VirtualFile schemaFile) {
		...
		switch(schemaType) {
			...
			case embeddedSchema:
				...
				break;
			case remoteSchema:
				// this works only for user-specified remote schemas in our settings, but not for auto-detected remote
				// schemas
				JsonSchemaUsageTriggerCollector.trigger(REMOTE_USAGE_KEY);
				break;
		}
	...
	private static class Worker {
		...
		public void work() {
			...
			final Collection<JsonSchemaObject> schemas =
				new JsonSchemaResolver(myProject, myRootSchema, position).resolve();
			...
			schemas.forEach(schema -> {
				if(isName != ThreeState.NO) {
					...
					final Collection<String> properties =
						myWalker.getPropertyNamesOfParentObject(myOriginalPosition, myPosition);
			...
			Set<String> knownNames)
		{
			...
			for(IfThenElse ifThenElse : ifThenElseList) {
				try {
					var constructor =  JsonSchemaAnnotatorChecker.class.getConstructor(Project.class,JsonComplianceCheckerOptions.class);
					constructor.trySetAccessible();
					JsonSchemaAnnotatorChecker checker = constructor.newInstance(myProject, JsonComplianceCheckerOptions.RELAX_ENUM_CHECK);
		...
		private void addAllPropertyVariants(boolean insertComma,
			...
			schemaProperties.keySet().stream()
				.filter(name -> !properties.contains(name) && !knownNames.contains(name) || adapter != null && name
					.equals(adapter.getName()))
				.forEach(name -> {
					knownNames.add(name);
		...
		private void suggestValues(JsonSchemaObject schema, boolean isSurelyValue) {
			...
			if(schema.getEnum() != null) {
				...
				for(Object o : schema.getEnum()) {
					if(myInsideStringLiteral && !(o instanceof String))
						continue;
					String variant = o.toString();
					if(!filtered.contains(variant)) {
						Map<String, String> valueMetadata =
							metadata == null ? null : metadata.get(StringUtil.unquoteString(variant));
						...
						addValueVariant(variant, description,
							deprecated != null ? (variant + " (" + deprecated + ")") : null, null);
					}
				}
			...
			if(JsonSchemaVersion.isSchemaSchemaId(myRootSchema.getId()) && type == JsonSchemaType._string) {
				...
				if(name.equals("required")) {
					...
					if(parent != null) {
						...
						if(adapter != null && JsonSchemaObject.X_INTELLIJ_LANGUAGE_INJECTION
							.equals(adapter.getName())) {
							addInjectedLanguageVariants();
						}
					}
		...
		private void addValueVariant(@NotNull String key,
			...
			LookupElementBuilder builder =
				LookupElementBuilder.create(!shouldWrapInQuotes(unquoted, true) ? unquoted : key);
			if(altText != null) {
		...
		private void addPropertyVariant(@NotNull String key,
			...
			boolean hasValue,
			boolean insertComma) {
			final Collection<JsonSchemaObject> variants =
				new JsonSchemaResolver(myProject, jsonSchemaObject).resolve();
			...
			if(!StringUtil.isEmptyOrSpaces(typeText)) {
				...
				builder = builder
					.withTypeText(firstSentenceMark == -1 ? text : text.substring(0, firstSentenceMark + 1), true);
			} else {
		...
		private InsertHandler<LookupElement> createDefaultPropertyInsertHandler(
			@SuppressWarnings("SameParameterValue") boolean hasValue,
			boolean insertComma) {
			return new InsertHandler<LookupElement>() {
				@Override
				public void handleInsert(@NotNull InsertionContext context, @NotNull LookupElement item) {
					...
					if(handleInsideQuotesInsertion(context, editor, hasValue))
						return;
					...
					if(hasValue) {
						...
						if(offset < docChars.length() && docChars.charAt(offset) != SEPARATOR_CHAR) {
							editor.getDocument().insertString(initialOffset, SEPARATOR);
					...
					if(offset < docChars.length() && docChars.charAt(offset) == SEPARATOR_CHAR) {
						...
						final String stringToInsert = SEPARATOR + "1" + (insertComma ? "," : "");
		...
		private InsertHandler<LookupElement> createPropertyInsertHandler(@NotNull JsonSchemaObject jsonSchemaObject,
			...
			final String defaultValueAsString =
				defaultValue == null || defaultValue instanceof JsonSchemaObject ? null :
			...
			return new InsertHandler<LookupElement>() {
				...
				public void handleInsert(@NotNull InsertionContext context, @NotNull LookupElement item) {
					...
					boolean insertColon = element == null || !SEPARATOR.equals(element.getText());
					if(!insertColon) {
					...
					if(finalType != null) {
						...
						switch(finalType) {
							case _object:
								EditorModificationUtil.insertStringAtCaret(editor, insertColon ? SEPARATOR : " ",
									false, true,
									insertColon ? 2 : 1);
								hadEnter = false;
							...
							case _boolean:
								...
								stringToInsert = (insertColon ? SEPARATOR : " ") + value + comma;
		...
		private static void invokeEnterHandler(Editor editor) {
			...
			handler.execute(editor, caret,
				new CaretSpecificDataContext(DataManager.getInstance().getDataContext(editor.getContentComponent()),
					caret));
		}

		private boolean handleInsideQuotesInsertion(@NotNull InsertionContext context, @NotNull Editor editor,
			boolean hasValue) {
			if(myInsideStringLiteral) {
				int offset = editor.getCaretModel().getOffset();
	...
	private static void insertPropertyWithEnum(InsertionContext context,
		...
		final boolean isNumber =
			type != null && (JsonSchemaType._integer.equals(type) || JsonSchemaType._number.equals(type)) ||
