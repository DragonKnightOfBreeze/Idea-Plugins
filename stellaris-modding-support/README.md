...
* ［已解决］只有在包含descriptor.mod文件的文件夹下的相关文件才应被识别为本地化文件和脚本文件
* ［已解决］Json Schema进行验证时，Json字符串是带双引号的
* ［未解决］脚本文件的键值也可以加双引号（？？？）
* ［未解决］脚本文件的语法解析比较慢（虽然很快就能正确地适用代码高亮）
* ［未解决］jar包中的Schema文件的跨文件规则无法正常识别的BUG
* ［未解决］存在其他合法的颜色码，但是这个插件认为是不支持的
* ［未解决］通过快速修正更改了文件编码和BOM之后，必须真正编辑文件内容才会进行下一次检查，让上方的警告消失
